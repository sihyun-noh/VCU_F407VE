...
static void DAC_Config(void)
{
	...
	/* 使能GPIOA时钟 */
	RCC_AHB1PeriphClockCmd(DAC_CH1_GPIO_CLK|DAC_CH2_GPIO_CLK|RCC_AHB1Periph_GPIOB, ENABLE);	
...
static void DAC_TIM_Config(void)
{
	...
	/* 使能TIM2时钟，TIM2CLK 为84M  （存疑感觉为42M） */
	...
	/*使用定时器的更新事件触发DAC，每隔84/20 = 4.2MHZ的周期触发一次dac事件 dac进行数模转换后由DMA将数据
	搬运到相应的输出数据寄存器输出，因为输出32个数据完成后算是输出了一个周期，那么输出一个周期所用的频率约为 4.2/32 = 131KHZ*/
