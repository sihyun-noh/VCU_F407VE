...
/* ===================== Helpers ===================== */
static inline rt_tick_t now_tick(void) { return rt_tick_get(); }
...
{
    if (rx->ext_id != CANID_UPPER_CMD_RX ) return false;
...
{
    if (rx->ext_id != CANID_UPPER_CMD_RPM_RX) return false;
...
{
    if (rx->ext_id != CANID_MOTOR_STATUS_RX) return false;
...
static void fsm_thread_entry(void *parameter)
{
    ...
    for (;;)
    {
        ...
        out_st.flags = 0;
        if (rc.rc_enable) out_st.flags |= (1u<<0);
        if (rc.rc_emergency_stop) out_st.flags |= (1u<<1);
        if (upper_force_stop) out_st.flags |= (1u<<2);
        ...
        if (upper_force_stop)
        {
            ...
            out_st.control_src = 0; out_st.stop_reason = 1;
        }
        else if (rc_emg)
        {
            //out_cmd.src = SRC_NONE; out_cmd.type = CMD_STOP;
						out_cmd_F.src = SRC_NONE; out_cmd_F.type = CMD_STOP;
						out_cmd_R.src = SRC_NONE; out_cmd_R.type = CMD_STOP;
            out_st.control_src = 0; out_st.stop_reason = 2;
        }
        else if (!motor_F_ok)
        {
            //out_cmd.src = SRC_NONE; out_cmd.type = CMD_STOP;
        ...
        else
        {
            ...
            if (rc_ok && rc.rc_enable)
            {
                ...
                //out_cmd.src = SRC_RC; out_cmd.type = CMD_SETPOINT;
								out_cmd_F.src = SRC_RC; out_cmd_F.type = CMD_SETPOINT;
								out_cmd_R.src = SRC_RC; out_cmd_R.type = CMD_SETPOINT;
                //out_cmd.rpm_axis1 = rc.axis1; out_cmd.rpm_axis2 = rc.axis2;
								out_cmd_F.rpm_axis1 = rc.axis1; out_cmd_F.rpm_axis2 = rc.axis3;
								out_cmd_R.rpm_axis1 = rc.axis1; out_cmd_R.rpm_axis2 = rc.axis3;
                
								out_st.control_src = 1; out_st.stop_reason = 0;
            }
            else if (upper_ok)
            {
                ...
                if (upper.upper_force_active == 0)
                {
                    ...
                    out_cmd_F.rpm_axis1 = 0; out_cmd_F.rpm_axis2 = 0;
									  out_cmd_R.type = CMD_STOP;
                    out_cmd_R.rpm_axis1 = 0; out_cmd_R.rpm_axis2 = 0;
                ...
                else
                {
                   ...
                   out_cmd_F.rpm_axis1 = upper_rpm.driver_f_axis1_rpm; out_cmd_F.rpm_axis2 = upper_rpm.driver_f_axis2_rpm;
									 out_cmd_R.type = CMD_SETPOINT;
                   out_cmd_R.rpm_axis1 = upper_rpm.driver_r_axis1_rpm; out_cmd_R.rpm_axis2 = upper_rpm.driver_r_axis2_rpm;
									
                }
                out_st.control_src = 2; out_st.stop_reason = 0;
            }
            else
            {
                ...
                out_cmd_F.src = SRC_NONE; out_cmd_F.type = CMD_STOP;
								out_cmd_R.src = SRC_NONE; out_cmd_R.type = CMD_STOP;
                out_st.control_src = 0; out_st.stop_reason = 4;
            }
        }
