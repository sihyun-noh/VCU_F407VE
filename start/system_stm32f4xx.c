...
Èç¹ûÍâ²¿¾§ÕñÊ¹ÓÃ8MHZ¾§Õñ ÐèÒª½«¸Ã¹«Ê½ÁªºÏÏÂÃæ¹«Ê½ ¾­¼ÆËãºóÏµÍ³Ê±ÖÓÒªÎª168MHz
*/ 
#define PLL_M      8
#else /* STM32F411xE */
#if defined (USE_HSE_BYPASS)
...
//ÔÚÕâÖ®Ç°µÄÕâÐ©²ÎÊýÅäÖÃ²»Í¬¿É¶ÔF407µÄÏµÍ³Ê±ÖÓÆµÂÊ½øÐÐ²»Í¬µÄÅäÖÃ£¬µ«Ò»°ãÊÇÅäÖÃÎª168MHZ
//PLL_VCO = (8000000/8)*336    PLL_VCO = 336000000
//SYSCLK  = 336000000/2        SYSCLK  = 168000000
...
#if defined (STM32F40_41xxx)
  uint32_t SystemCoreClock = 168000000;								//Ö»ÊÇ¸øÒ»¸ö³õÊ¼µÄÖ÷ÆµÖµ²»Ò»¶¨ÕæÊµ£¬¾ßÌåÖ÷Æµ¿ÉÒÔÍ¨¹ýÄÇÁ½¸ö¹«Ê½ÉèÖÃ£¬È»ºóÍ¨¹ý¸üÐÂº¯Êý½øÐÐÖ÷Æµ¸üÐÂ£¬ÔÙ´Î¶ÁÕâ¸öÖµ±ãÊÇ×¼È·µÄÖ÷ÆµÖµ
#endif /* STM32F40_41xxx */
...
void SystemInit(void)
{
  ...
  /* Set HSION bit */   //¿ªÆôHSI£¬×î¿ªÊ¼ÆôÓÃµÄÊÇÄ¬ÈÏµÄ¸ßËÙÄÚ²¿Ê±ÖÓ
...
/******************************************************************************/
  ...
  /* Enable HSE */  //Ê¹ÄÜÍâ²¿¸ßËÙÊ±ÖÓ
  RCC->CR |= ((uint32_t)RCC_CR_HSEON);
 
  //µÈ´ý£¬Ö±µ½HSE×¼±¸ºÃ£¬Èç¹û³¬Ê±ÔòÍË³ö
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    /* HCLK = SYSCLK / 1*/ //AHB²»·ÖÆµ	ÆäÊµAHBµÄÊ±ÖÓÒ²ÊÇÏµÍ³Ö÷Ê±ÖÓ£¬ÏµÍ³ÆµÂÊÊÇÓÉAHB×ÜÏß¸øµ½ÄÚºËµÄ£¬¹¹³ÉÏµÍ³Ö÷Æµ¡£
    RCC->CFGR |= RCC_CFGR_HPRE_DIV1;

#if defined (STM32F40_41xxx) || defined (STM32F427_437xx) || defined (STM32F429_439xx)      
    /* PCLK2 = HCLK / 2*/  //APB2ÓÉAHBµÄÊ±ÖÓ½øÐÐ2·ÖÆµ
    RCC->CFGR |= RCC_CFGR_PPRE2_DIV2;
    
    /* PCLK1 = HCLK / 4*/  //APB1ÓÉAHBµÄÊ±ÖÓ½øÐÐ4·ÖÆµ  ¿ÉÍ¨¹ý¸Ä±äÕâ²¿·Ö´úÂëµÄ·ÖÆµÖµÈ¥ÉèÖÃAPB1,2ºÍAHB×ÜÏßµÄÊ±ÖÓÆµÂÊ
    ...
    RCC->CFGR |= RCC_CFGR_PPRE1_DIV2;
																							...
																							/*
																							ÅäÖÃËøÏà»·µ½AHB×ÜÏß²úÉúÏµÍ³Ê±ÖÓÎª168MHz,ÓÐÏÂÃæÁ½¸ö¹«Ê½½øÐÐ²ÎÊýÅäÖÃ
																							PLL_VCO = (HSE_VALUE or HSI_VALUE / PLL_M) * PLL_N 
																							SYSCLK = PLL_VCO / PLL_P
																							¸ù¾ÝÕâÁ½¸ö¹«Ê½È¥ÉèÖÃÕâÀïÃæ±äÁ¿µÄ²ÎÊýµÄÖµ£¬ÕâÐ©±äÁ¿¿ÉÒÔÔÚÉÏÃæ½øÐÐÉèÖÃ
																							¿ÉÒÔÍ¨¹ýÉèÖÃ±äÁ¿²»Í¬µÄ²ÎÊýÖµÈ¥¸Ä±äAHB×ÜÏßÊ±ÖÓ£¬´Ó¶ø¸Ä±äÏµÍ³Ö÷ÆµµÄ´óÐ¡
																							*/
	...
	//Ñ¡ÔñËøÏà»·Êä³ö³ÉÎªÏµÍ³Ê±ÖÓÔ´
    /* Select the main PLL as system clock source */
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_SW));
    RCC->CFGR |= RCC_CFGR_SW_PLL;
	
	//µÈ´ýËøÏà»·µÄÊä³ö³ÉÎªÏµÍ³Ê±ÖÓÔ´
