...
#include "delay.h"
#include "main.h"
int8_t RS232_2_flag = 0;

//如果发送接收拿不到数据一般都是初始化参数的问题
void Init_RS232_2(void)
{
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA,ENABLE);										//开启GPIOA 和USART2 的时钟
	...
	GPIO_PinAFConfig(GPIOA, GPIO_PinSource2, GPIO_AF_USART2);									//将GPIOA的10,11引脚复用为USART4
	...
	Init_RS232_2_NVIC();																				//初始化NVIC
	
	//第二个参数配置出错1次，会导致进中断出不来 串口使用中断接收数据要用接收的标志位寄存器作连接
	USART_ITConfig(USART2,USART_IT_RXNE,ENABLE);												//开启NVIC
	
	USART_Cmd(USART2,ENABLE);																	//给uart上电 使能USART2
}
...
void RS232_2_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i ++)
	{
		RS232_2_SendByte(Number / RS232_2_Pow(10, Length - i - 1) % 10 + '0');
	}
}
...
//串口1的接收中断
 void USART2_IRQHandler(void)																		//重写串口接收中断 接收串口收到的数据
{	
