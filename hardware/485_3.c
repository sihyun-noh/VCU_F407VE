...
#include "485_3.h"
#include "delay.h"
int8_t RS485_3_flag = 0;
//如果发送接收拿不到数据一般都是初始化参数的问题
void Init_RS485_3(void)
{
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB,ENABLE);										//开启GPIOB 和USART3 的时钟
	...
	GPIO_PinAFConfig(GPIOB, GPIO_PinSource10, GPIO_AF_USART3);									//将GPIOB的10,11引脚复用为USART4
	...
	Init_RS485_3_NVIC();																				//初始化NVIC
	
	//第二个参数配置出错1次，会导致进中断出不来 串口使用中断接收数据要用接收的标志位寄存器作连接
	USART_ITConfig(USART3,USART_IT_RXNE,ENABLE);												//开启NVIC
	
	USART_Cmd(USART3,ENABLE);																	//给uart上电 使能USART3
}
...
void RS485_3_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i ++)
	{
		RS485_3_SendByte(Number / RS485_3_Pow(10, Length - i - 1) % 10 + '0');
	}
}
...
//串口1的接收中断
 void USART3_IRQHandler(void)																		//重写串口接收中断 接收串口收到的数据
{	
